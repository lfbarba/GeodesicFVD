...
%Once the funnel $\funnel{r\to C_r}$ is computed, we will compute later the geodesic distance function from $r$ only inside of this funnel.
...
%
%\begin{restatable}{lemma}{SPMForSitesR}
...
\PropertiesOfQ*
\begin{proof}\label{ProofPropertiesOfQ}
\hyperref[lemma:PropertiesOfQ]{$\hookleftarrow$}
The boundary of $Q$ can be constructed by connecting consecutive  points in $V_B\cup V_C$ by geodesics contained in the complement of $P$, i.e., in a domain that has $P$ as a hole or obstacle ($Q$ is also known as the relative hull of $V_B\cup V_C$ in this domain). 
...
\newcommand{\ProofSPMForSitesR}{
\SPMForSitesR*  
\begin{proof}\label{ProofSPMForSitesR}
\hyperref[lemma:SPMForSitesR]{$\hookleftarrow$}
...
\newcommand{\ProofShadowPoints}{
\ShadowPoints*  
\begin{proof}\label{ProofShadowPoints}
\hyperref[lemma:Shadow points]{$\hookleftarrow$}
...
\begin{restatable}{lemma}{PatchContainsVcell}\label{lemma:Patch contains vcell} 
\hyperref[ProofPatchContainsVcell]{$[*]$}
For each $0\leq i \leq |R|$ and for each site $s\in \s_i$, 
each point in the Voronoi cell $\cell{s, \s}$ is \idom by $s$.
\end{restatable}

\newcommand{\ProofPatchContainsVcell}{
\PatchContainsVcell*  
\begin{proof}\label{ProofPatchContainsVcell}
\hyperref[lemma:Patch contains vcell]{$\hookleftarrow$}
...
\newcommand{\ProofStructureEnvelope}{
\StructureEnvelope*
\begin{proof}\label{ProofStructureEnvelope}
\hyperref[lemma:Structure of the envelope]{$\hookleftarrow$}
...
\newcommand{\ProofTimeInsertionR}{
\TimeInsertionR*
\begin{proof}\label{ProofTimeInsertionR}
\hyperref[lemma:Time to insert one site of R]{$\hookleftarrow$}
...
After moving, we can redefine $H$ and repeat the process until finding an endpoint $z$ of the intersection of $\partial Q$ with the \icell of $r$; see Figure~\ref{fig:RrefinedDiagram}. 
...
\newcommand{\ProofComplexityEnvelope}{
\ComplexityEnvelope*
\begin{proof}\label{ProofComplexityEnvelope}
\hyperref[lemma: Complexity of envelope-graph]{$\hookleftarrow$}
...
\newcommand{\ProofFinalTheorem}{
\FinalTheorem*
\begin{proof}\label{ProofFinalTheorem}
\hyperref[thm:Final Theorem]{$\hookleftarrow$}
By Lemma~\ref{lemma:Patch contains vcell}, the \icell[|R|] of each site $s$ of $\s$ contains its corresponding Voronoi cell $\cell{s, \s}$. 
Because the union of these Voronoi cells covers $P$, we conclude that the \icell[|R|] of $s$ and $\cell{s, \s}$ coincide inside $P$ for each $s\in \s$.
That is, the envelope-graph of $\s_{|R|}$ coincides with the Voronoi tree of $\vd{\s}$.
...
Finally, since an arc can only be created once, then the total number of arcs that disappear is at most the number of arcs created, which is the number of arcs in $\vd[Q]{B}$, plus the arcs accounted in each $M_r$. Thus, by our previous arguments $\ex{\sum_{r\in R} D_r}  \leq  O(n) + \ex{\sum_{b\in B} |\mu_b|} = O(n)$.
